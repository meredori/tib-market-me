...
exports.computeSnapshotPricing = computeSnapshotPricing;
...
function computeSnapshotPricing(row) {
    const anchors = [];
    ...
    const lowerCandidates = [valid(row.month_lowest_sell), valid(row.day_lowest_sell), buyOffer].filter((value) => value !== null);
    const upperCandidates = [valid(row.month_highest_sell), valid(row.day_highest_sell), sellOffer].filter((value) => value !== null);
