...
exports.runMarketSync = runMarketSync;
exports.getStatus = getStatus;
exports.searchLatestItems = searchLatestItems;
exports.preflightRefresh = preflightRefresh;
exports.getItemDetails = getItemDetails;
const node_crypto_1 = __importDefault(require("node:crypto"));
const node_fs_1 = __importDefault(require("node:fs"));
const node_path_1 = __importDefault(require("node:path"));
const config_1 = require("../../config");
const snapshotPricing_1 = require("../pricing/snapshotPricing");
const client_1 = require("../tibiamarket/client");
const searchStatementsByDb = new WeakMap();
...
function bestNpcBuyPrice(metaRow) {
...
function selectedWorld(worldRows, serverName) {
...
function buildLootLogicPreview(row) {
...
function exportJsonFiles(payload) {
    ensureParentDir(config_1.config.outputFlatPath);
    ensureParentDir(config_1.config.outputDetailPath);
    ensureParentDir(config_1.config.outputMetaPath);
    ...
    node_fs_1.default.writeFileSync(config_1.config.outputFlatPath, `${JSON.stringify(sortedFlat, null, 2)}\n`, "utf-8");
    const detailPayload = {
        generated_at: payload.runFinishedAt,
        server: config_1.config.serverName,
        pricing_model: config_1.config.pricingModelVersion,
        sales_tax_pct: config_1.config.salesTaxPct,
        item_count: payload.detailedRows.length,
        items: payload.detailedRows
    };
    node_fs_1.default.writeFileSync(config_1.config.outputDetailPath, `${JSON.stringify(detailPayload, null, 2)}\n`, "utf-8");
    const metaPayload = {
        server: config_1.config.serverName,
        local_run: {
        ...
        world_data: {
            ...
            server: asText(payload.worldRow?.name) || config_1.config.serverName,
            last_update: asText(payload.worldRow?.last_update) || null
        },
        outputs: {
            flat_prices: config_1.config.outputFlatPath,
            detailed_prices: config_1.config.outputDetailPath
        },
        counts: {
    ...
    node_fs_1.default.writeFileSync(config_1.config.outputMetaPath, `${JSON.stringify(metaPayload, null, 2)}\n`, "utf-8");
}
async function runMarketSync(db) {
    const client = new client_1.TibiaMarketClient();
    const runStartedAt = nowIso();
    console.log(`[sync] starting market sync for ${config_1.config.serverName}`);
  ...
  `);
    const runInsertInfo = insertRun.run(config_1.config.serverName, runStartedAt, runStartedAt, runStartedAt, null, runStartedAt, config_1.config.pricingModelVersion, config_1.config.salesTaxPct, config_1.config.pageLimit, config_1.config.pagePauseMs / 1000, 0, 0);
    ...
    const [marketRows, metadataRows, worldRows] = await Promise.all([
        client.getAllMarketValues(config_1.config.serverName),
        client.getItemMetadata(),
        client.getWorldData(config_1.config.serverName)
    ]);
    console.log(`[sync] fetched ${marketRows.length} market rows, ${metadataRows.length} metadata rows, ${worldRows.length} world rows`);
  ...
  `);
    const storeMetadata = db.transaction(() => {
        ...
        for (const meta of metadataRows) {
            ...
            metadataUpsert.run(meta.id, asText(meta.name) || null, asText(meta.wiki_name) || null, asText(meta.category) || null, asInt(meta.tier, -1), rawJson, sha256(rawJson), fetchedAt, fetchedAt);
            deleteNpcBuy.run(meta.id);
            deleteNpcSell.run(meta.id);
            const npcBuyByKey = new Map();
            const npcSellByKey = new Map();
            ...
            for (const row of npcBuyByKey.values()) {
                insertNpcBuy.run(meta.id, asText(row.npc_name), asText(row.location), asInt(row.price, 0), asInt(row.currency_object_type_id, 0), asText(row.currency_quest_flag_display_name), fetchedAt);
            }
            for (const row of npcSellByKey.values()) {
                insertNpcSell.run(meta.id, asText(row.npc_name), asText(row.location), asInt(row.price, 0), asInt(row.currency_object_type_id, 0), asText(row.currency_quest_flag_display_name), fetchedAt);
            }
            metadataCount += 1;
        }
    ...
    const worldRow = selectedWorld(worldRows, config_1.config.serverName);
    if (worldRow) {
        console.log(`[sync] storing world freshness snapshot for ${asText(worldRow.name) || config_1.config.serverName}`);
        db.prepare("INSERT INTO world_data_snapshots(server, last_update, fetched_at) VALUES (?, ?, ?)").run(asText(worldRow.name) || config_1.config.serverName, asText(worldRow.last_update), worldFetchedAt);
    }
  ...
  `);
    ...
    const storeMarketRows = db.transaction(() => {
        ...
        for (const row of marketRows) {
            ...
            const pricing = (0, snapshotPricing_1.computeSnapshotPricing)(row);
            const npcBuy = bestNpcBuyPrice(meta);
            const expectedNet = Math.round(Math.max(pricing.suggested_list_price, pricing.fair_price, 0) * (1 - config_1.config.salesTaxPct / 100));
            ...
            insertFeatures.run(runId, itemId, upstreamTime, isFullData, asInt(row.buy_offer, -1), asInt(row.sell_offer, -1), asInt(row.month_average_sell, -1), asInt(row.month_average_buy, -1), asInt(row.month_sold, -1), asInt(row.month_bought, -1), asInt(row.active_traders, -1), asInt(row.month_highest_sell, -1), asInt(row.month_lowest_buy, -1), asInt(row.month_lowest_sell, -1), asInt(row.month_highest_buy, -1), asInt(row.buy_offers, -1), asInt(row.sell_offers, -1), asInt(row.day_average_sell, -1), asInt(row.day_average_buy, -1), asInt(row.day_sold, -1), asInt(row.day_bought, -1), asInt(row.day_highest_sell, -1), asInt(row.day_lowest_sell, -1), asInt(row.day_highest_buy, -1), asInt(row.day_lowest_buy, -1), asInt(row.total_immediate_profit, -1), asText(row.total_immediate_profit_info));
            insertPrice.run(runId, itemId, config_1.config.pricingModel, config_1.config.pricingModelVersion, pricing.fair_price, pricing.suggested_list_price, clientValue, pricing.trend, pricing.trend_score, pricing.liquidity, pricing.confidence);
            detailedRows.push({
                id: itemId,
    ...
    SET
      ...
      priced_item_count = ?
    WHERE id = ?
  `).run(runFinishedAt, runFinishedAt, worldLastUpdate, worldFetchedAt, marketRows.length, Object.keys(flatPrices).length, runId);
    exportJsonFiles({
        flatPrices,
        detailedRows,
        runStartedAt,
...
function getStatus(db) {
    ...
    `)
        .get();
    return {
        server: run?.server ?? config_1.config.serverName,
        local_run: {
        ...
        files: {
            prices: config_1.config.outputFlatPath,
            detail: config_1.config.outputDetailPath,
            meta: config_1.config.outputMetaPath
        }
    };
}
function searchLatestItems(db, query, limit = 80) {
    ...
    if (!statements) {
        statements = {
            latestRun: db.prepare("SELECT id FROM market_runs ORDER BY id DESC LIMIT 1"),
            search: db.prepare(`
        WITH matched AS (
          SELECT
            mip.item_id AS id,
    ...
    const rows = statements.search.all(latestRun.id, config_1.config.pricingModel, likePattern, likePattern, likePattern, limit);
    return rows.map((row) => {
...
async function preflightRefresh(db) {
    const status = getStatus(db);
    const currentLastUpdate = status.world_data.last_update;
    const client = new client_1.TibiaMarketClient();
    const worldRows = await client.getWorldData(config_1.config.serverName);
    const remoteWorld = selectedWorld(worldRows, config_1.config.serverName);
    ...
    const isNewer = remoteTs !== null && currentTs !== null
        ? remoteTs > currentTs
...
function getItemDetails(db, itemId) {
    ...
    const detail = db
      ...
      LIMIT 1
    `)
        .get(latestRun.id, config_1.config.pricingModel, itemId);
    if (!detail) {
        return null;
    }
    const npcBuyRows = db
