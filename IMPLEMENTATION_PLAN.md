...
- Hunt ingestion should be rebuilt around structured hunt-session imports such as `huntanalyser.sample.txt`.
...
- Raw SQL migrations fit the schema-driven approach already documented in `SQLITE_SCHEMA.md`.
...
- Keep the existing sample files as migration fixtures:
  ...
  - `samplelog.txt`
  - `samplelog.recommendation.json`
  - `api_recommend.sample.json`
  - `huntanalyser.sample.txt`
...
- Use `huntanalyser.sample.txt` as the primary example of the desired ingestion format.
- Parse and persist:
  - session start/end
  - session duration
  - XP gain and XP/h
